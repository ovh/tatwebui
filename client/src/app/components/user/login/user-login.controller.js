 ...
 *
 */
angular.module('TatUi').controller('UserLoginCtrl', function ($scope, $rootScope, $state, Authentication, appConfiguration) {
    'use strict';
    ...
    $scope.connect = function() {
        Authentication.connect($scope.user).then(function() {
            $rootScope.$broadcast('topic-change', {topic:'Private/' + $scope.user.username});
        }, function(err) {
            console.log(err);
            $state.go('index');
        });
