...
module.exports = function (grunt) {
    ...
    grunt.initConfig({
        ...
        watch: {
            dev: {
                files: ['<%= project.src%>/index.html', '<%= project.app%>/**/*.*', '<%= project.bower%>/tatwebui-plugin-*/**/*.*'],
                tasks: [
                    'jshint',
                    'less',
                    'xml2json',
        ...
        less: {
            dist: {
                options: {},
                files: {
                    '<%= project.build%>/main.css': ['<%= project.app%>/**/*.less','<%= project.build%>/tatwebui-plugin-*/**/*.less']
                }
            }
        },
                ...
                files: [
                    {
                        ...
                        flatten: false,
                        cwd: '<%= project.bower%>',
                        src: ['tatwebui-plugin-*/**/*.html','tatwebui-plugin-*/**/*.less','tatwebui-plugin-*/**/*.js'],
                        dest: '<%= project.build%>/',
                        filter: 'isFile'
                    }
                ]
